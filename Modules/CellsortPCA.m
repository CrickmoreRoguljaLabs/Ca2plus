function [mixedsig, mixedfilters, CovEvals, covtrace, movm, ...
    movtm] = CellsortPCA(fn, flims, nPCs, dsamp, outputdir, badframes)%, polyfocus)
...
if nt < npix
    [covmat, mov, movm, movtm] = create_tcov(fn, pixw, pixh, useframes, nt, dsamp);%, polyfocus);
else
    [covmat, mov, movm, movtm] = create_xcov(fn, pixw, pixh, useframes, nt, dsamp);%, polyfocus);
end
...
toc
    ...
    function [covmat, mov, movm, movtm] = create_xcov(fn, pixw, pixh, useframes, nt, dsamp)%, polyfocus)
        %-----------------------
        ...
        if (dsamp_space==1)
            ...
            for jjind=1:length(useframes)
                jj = useframes(jjind);
                tempread=imread(fn,jj);
                mov(:,:,jjind) = double(tempread);%.*polyfocus;
                if mod(jjind,500)==1
        ...
        movm = mean(mov,2); % Average over time % No, subtraction mean instead
    ...
    function [covmat, mov, movm, movtm] = create_tcov(fn, pixw, pixh, useframes, nt, dsamp)%, polyfocus)
        %-----------------------
        ...
        if (dsamp_space==1)
            ...
            for jjind=1:length(useframes)
                jj = useframes(jjind);
                tempread= imread(fn,jj);
                mov(:,:,jjind) = double(tempread);%polyfocus.*double(tempread);
                if mod(jjind,500)==1
                    fprintf(' Read frame %4.0f out of %4.0f; ', jjind, nt)
                    toc
                end
